...
class battery_portfolio:
    def __init__(self, ns, dam_participation, rm_participation, fm_participation, bm_participation, dam_prices, rm_up_prices, rm_dn_prices, fmp_prices, fmq_prices, bm_up_prices, bm_dn_prices,
        ...
        self.dam_participation = dam_participation
        # (1 if the BSUs participate in the RM, 0 otherwise)
        self.rm_participation = rm_participation
        # (1 if the BSUs participate in the FM, 0 otherwise)
        self.fm_participation = fm_participation
        # (1 if the BSUs participate in the BM, 0 otherwise)
        self.bm_participation = bm_participation
        ...
        # BSUs' rountrip efficiencies (0 to 1, unitless)
    ...
    def maximize_stacked_revenues(self):
        ...
        model.chUB = pyo.Constraint(model.S, model.T, rule=lambda model,
                                    i, t: model.ch[i, t] <= self.S_max[i]*(1-model.u[i, t]))

        model.rupUB = pyo.Constraint(model.S, model.T, rule=lambda model, i,
                                     t: model.rup[i, t] <= self.S_max[i] + model.ch[i, t] - model.dis[i, t])

        model.rdnUB = pyo.Constraint(model.S, model.T, rule=lambda model, i,
                                     t: model.rdn[i, t] <= self.S_max[i] - model.ch[i, t] + model.dis[i, t])

        model.pupUB = pyo.Constraint(model.S, model.T, rule=lambda model, i,
                                     t: model.pup[i, t] <= self.S_max[i] + model.ch[i, t] - model.dis[i, t] - model.rup[i, t])

        model.pdnUB = pyo.Constraint(model.S, model.T, rule=lambda model, i,
                                     t: model.pdn[i, t] <= self.S_max[i] + model.dis[i, t] - model.ch[i, t] - model.rdn[i, t])
        ...
        model.rdnCapability = pyo.Constraint(
            model.S, model.T, rule=lambda model, i, t: model.soe[i, t] + model.rdn[i, t]*ch_eff[i] <= self.E_max[i])

        model.rupCapability = pyo.Constraint(
        ...
        model.lineCircleLinearization = pyo.Constraint(
            model.S, model.T, model.M, rule=lambda model, i, t, k: A_theta[k, i]*model.pess[i, t] + B_theta[k, i]*model.qess[i, t] <= C_theta[k, i])

        # Market Participation
        if self.dam_participation == 0:
            model.fix_dis = pyo.Constraint(
                model.S, model.T, rule=lambda model, i, t: model.dis[i, t] == 0)
            model.fix_ch = pyo.Constraint(
                model.S, model.T, rule=lambda model, i, t: model.ch[i, t] == 0)

        if self.rm_participation == 0:
            model.fix_rup = pyo.Constraint(
                model.S, model.T, rule=lambda model, i, t: model.rup[i, t] == 0)
            model.fix_rdn = pyo.Constraint(
                model.S, model.T, rule=lambda model, i, t: model.rdn[i, t] == 0)

        if self.fm_participation == 0:
            model.fix_qess = pyo.Constraint(
                model.S, model.T, rule=lambda model, i, t: model.qess[i, t] == 0)

        if self.fm_participation == 0 and self.bm_participation == 0:
            model.fix_pup = pyo.Constraint(
        ...
        if self.rm_participation == 1:
            rm_profits = sum(self.rm_up_prices[t]*sum(model.rup[i, t] for i in model.S) for t in model.T)+sum(
                self.rm_dn_prices[t]*sum(model.rdn[i, t] for i in model.S) for t in model.T)
        else:
            rm_profits = 0

        if self.fm_participation == 1:
            fm_profits = sum(sum(self.fmp_prices[i, t]*(model.pup[i, t]-model.pdn[i, t]) for i in model.S)
                             for t in model.T)+sum(sum(self.fmq_prices[i, t]*model.qess[i, t] for i in model.S) for t in model.T)
        else:
            fm_profits = 0

        if self.bm_participation == 1:
            bm_profits = sum(self.bm_up_prices[t]*sum(model.pup[i, t] for i in model.S) for t in model.T)-sum(
                self.bm_dn_prices[t]*sum(model.pdn[i, t] for i in model.S) for t in model.T)
        else:
            bm_profits = 0
